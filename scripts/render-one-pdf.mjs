...
const ROOT = process.env.OWANBE_ROOT ?? join(dirname(fileURLToPath(import.meta.url)), "..");
const MD_PATH = join(ROOT, process.env.MD_REL ?? "docs/OWANBE-FIX-REPORT.md");
const OUT_PATH = join(ROOT, process.env.OUT_REL ?? "docs/OWANBE-FIX-REPORT.pdf");
const COPY_PATH = process.env.COPY_PATH ?? join(process.env.HOME ?? "/tmp", "Downloads/Owanbe-Fix-Report.pdf");
const DOC_DATE = process.env.DOC_DATE ?? "30 July 2026";
const DOC_TITLE = process.env.DOC_TITLE ?? "Owanbe Planner";
const DOC_SUBTITLE = process.env.DOC_SUBTITLE ?? "";
...
doc.setProperties({
  title: DOC_TITLE,
  subject: `Owanbe document — ${DOC_DATE}`,
});
...
function drawHeaderFooter() {
  ...
  doc.text(`Owanbe Planner v6 | ${DOC_DATE}`, MARGIN, FOOTER_Y);
...
function collectHeadings() {
  ...
  for (const line of lines) {
    const t = line.trim();
    if (t.startsWith("## ") && !t.startsWith("### ")) headings.push({ level: 1, text: t.slice(3) });
...
function renderTitlePage() {
  ...
  if (DOC_SUBTITLE) {
    y += 4;
    writeWrapped(DOC_SUBTITLE, 12, "normal");
  }
  y += 6;
  writeWrapped(`Document date: ${DOC_DATE}`, 11, "normal");
  y += 4;
  writeWrapped("Owanbe Planner v6 — internal / developer documentation", 10, "italic");
  y += 16;
